...
from pages.locators import BasketPageLocatorsOnProductPage, BookWasAddedToBasket, NameBook, \
    TheCostOfTheBasketIsNowField, TheCost, \
    ProductPageLocators
...
class ProductPage(BasePage):
    def add_to_basket(self):
        button = WebDriverWait(self.browser, 2).until(
            expected_conditions.element_to_be_clickable(BasketPageLocatorsOnProductPage.BASKET_BUTTON))
        button.click()

    def check_that_book_was_added_to_basket(self):
        book_name_in_message = WebDriverWait(self.browser, 2).until(
            expected_conditions.element_to_be_clickable(BookWasAddedToBasket.BOOK_NAME_IN_MESSAGE)).text
